...
- 보안·권한·동시성처럼 판단이 중요한 결정이 필요하면 직접 처리하지 말고 BLOCKED 또는 NEEDS_CONTEXT로 올린다(그 판단은 메인 Opus 세션 몫). **스펙이 명확해 보여도, 민감면(인증·권한·시크릿·환경변수·DB 마이그레이션)을 건드리는데 안전 결정이 빠졌으면(예: 인증 체크가 명시 안 된 endpoint) 그대로 받아쓰지 말고 BLOCKED로 올린다** — 빠진 안전 결정 자체를 표면화한다("스펙에 없으니 그냥 두자"로 넘기지 않는다).
