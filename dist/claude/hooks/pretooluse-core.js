...
//
// 🛑🛑 **6회차: 자르는 쪽과 읽는 쪽이 서로의 전제를 몰라 구멍이 났다**(게이트가 실행으로 적발).
//   `2>/dev/null` 을 꼬리로 붙이면 `>` 에서 자르므로 **앞의 `2` 만 인자 구간에 남는다.** 그 `2` 를
//   아래 `RM_PATHISH` 의 `^[\w./~]` 갈래가 "경로 글자로 시작"으로 읽어 "지울 대상이 보인다"로
//   판단하고, **안전측 되돌림(온 명령 검사)이 아예 안 돌았다.** 실제로 뚫린 것 넷:
//     `find / -name '*.log' -exec rm -rf {} + 2>/dev/null`
//     `find / -type f | xargs rm -rf 2>/dev/null`
//     `cd / && rm -rf * 2>/dev/null`
//     `find / -exec rm -rf {} + 1>/dev/null`
//   그래서 **리다이렉션 앞 숫자(fd)까지 구간의 끝으로 본다.** 꼬리가 통째로 잘려 나가 인자 구간에는
//   자리표만 남고, 불투명 판정이 제 일을 한다.
//   ⚠ `\s*` 가 개행도 먹지만 **줄 이음 전제는 그대로 산다**: `\` 는 공백이 아니라 `\s*` 가 그 앞으로
//   못 넘어간다(아래 검사 두 줄이 그 자리를 붙든다). 이 줄을 고칠 때 그 두 줄을 함께 돌려라.
...
//   갈아치운다: **첫 글자가 경로 글자이거나(`^[\w./~]`), 어디든 슬래시를 품고 있으면(`[/]`)** 지울
//   대상으로 본다. 자리표(`{}`)·종결자(`+`·`;`·`\;`)·따옴표 조각(`'`·`"`)은 **둘 다 아니라서**
//   한꺼번에 불투명(= 온 명령 검사 = 안전측)으로 떨어진다. 처방의 핵심은 이 자리다.
//
// 🛑 **두 갈래를 다 넣어야 한다. 한쪽만 쓰면 각각 실측 오차단이 되살아난다**(5회차에 두 형태를
//   차례로 시험하고 둘 다 실행으로 기각했다 · 40개 사례 대조).
//   - `[/]` 만(슬래시를 품은 것만): `build`·`png`·`node_modules` 같은 **맨 이름 폴더**가 빠진다.
//     `rm -rf build && cd ..` 가 옆 자리 `cd ..` 때문에 막혔다(아래 검사 두 줄이 그 자리를 붙든다).
//     맨 이름은 그냥 상대 경로다.
//   - `^[\w./~]` 만(첫 글자만): **변수로 조립한 경로**가 빠진다. `$S/bt4`·`"$HOME/x"`·`${TMPDIR}/x`
//     는 첫 글자가 `$` 나 따옴표라 "대상이 아니다"로 떨어져 온 명령 검사로 되돌아간다.
//     이 판을 시작하게 만든 실사용 헛막음이 그 모양이었다(변수로 만든 임시 폴더 삭제).
const RM_PATHISH = /^[\w./~]|[/]/;
...
//     `rmHitsDangerTarget` 이 그 경우 **옛 판대로 온 명령을 본다**(위 `rmArgsOpaque` / `RM_PATHISH`). 정적으로
