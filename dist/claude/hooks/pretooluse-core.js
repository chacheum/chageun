...
// rm 재귀+강제(-rf·-fr·--recursive·--force)가 루트/홈/현재트리 등 위험 타깃을 지울 때.
// ⚠ **플래그를 띄어 쓴 `rm -r -f /` 는 안 걸린다**(2026-08-14 실행으로 확인 · 옛 판도 같다).
//   앞 주석이 `-r -f` 를 잡는다고 적어 놨는데 거짓이었다. 무늬는 한 낱말 안에 r 과 f 가 함께
//   있기를 요구한다. **정규식은 이 판에서 안 건드린다**(다른 축이고, 넓히면 재는 범위가 통째로
//   달라진다). 여기서는 거짓 보장만 지운다.
...
//   **자리(토큰)마다 무엇인지**를 묻는다. 갈래는 셋뿐이다:
//     "이름"   - 구체적인 대상을 가리킨다(`build` · `dist/*` · `$S/bt4`) → 이 rm 은 지울 곳을 밝혔다
//     "꼭대기" - 자리만 가리키고 이름이 없다(`/` · `~` · `$HOME` · `.` · `..` · `*` · `*/`) → 위험
//     그 밖   - 모른다(맨 변수 `$TARGET` · 자리표 `{}` · 따옴표 조각 · 종결자)
//   ⚠ **"모르면 곧 차단"이 아니다**(정직 회계). 모르는 표기가 가는 곳은 차단이 아니라 **옛 판
//   검사(온 명령에 `RM_DANGER_TARGET`)로 되돌아가는 것**이다. 그 검사가 온 명령에서 아무것도
//   못 찾으면 **통과한다**: `ls build | xargs rm -rf` 가 그 자리다. 안전측이라는 말은 "옛 판만큼은
//   본다"는 뜻이지 "다 막는다"는 뜻이 아니다.
//
...
// 🛑 **이름 글자에 비ASCII 를 반드시 넣는다**(7회차가 여기서 새 헛막음을 만들었다 · 실행으로 잡음).
//   `[\w-]` 는 ASCII 만 본다. 그래서 `rm -rf ./빌드결과` · `rm -rf ../빌드` · 이모지가 든 폴더가
//   "이름이 아니다"로 떨어져 `.`·`..` 만 남은 꼴이 되고 **꼭대기로 읽혀 막혔다.** 옛 판은 안 막던
//   것이라 순수한 손해였다. 한글·한자·키릴·이모지는 그냥 평범한 폴더 이름이다.
//   ⚠ **비ASCII 만 더한다.** 이 자리를 "구조 글자가 아니면 전부 이름"으로 뒤집으면 안 된다:
//   그러면 처음 보는 ASCII 기호가 이름으로 읽혀 **되돌림이 꺼지는 쪽**(= 여는 쪽)으로 틀린다.
//   자리표 `{}` · 종결자 `+`·`;`·`\` 는 전부 ASCII 라 지금대로 "모름"에 남는다.
const RM_SEG_NAME = /[\w-]|[^\x00-\x7F]/;
function rmSegKind(seg) {
  if (RM_SEG_TOP.test(seg)) return "top";
  if (seg.charAt(0) === "$") return null;      // 변수는 무엇이 들었는지 여기서 모른다
  if (RM_SEG_NAME.test(seg)) return "name";    // 이름 글자가 하나라도 있으면 구체적인 대상이다
...
const RM_GLOB = /[*?]/;
// 인자 한 자리가 무엇을 가리키나. `null` = 모른다(되돌림 후보) · `"skip"` = 대상 자리가 아니다.
//   ⚠ 플래그를 `null`(모름)과 갈라 두는 이유: 아래 되돌림 조건이 "모르는 자리가 하나도 없을 때만
//   끈다"라서, 플래그를 모름으로 세면 `rm -rf --one-file-system ./build` 같은 정상 줄이 늘 되돌아간다.
function rmTokenKind(rawTok) {
  const t = rmUnwrap(rawTok);
  if (!t || t.charAt(0) === "-") return "skip";      // 빈 조각 · 플래그 · `--`
...
// 🛑🛑 **자른 자리가 진짜 인자의 끝이 아닐 수 있다. 그때는 그 구간을 믿으면 안 된다.**
//   (7회차에 세 갈래를 스스로 열었고 셋 다 실행으로 잡았다. 전부 옛 판은 막던 것이다.)
//   구간 자르기를 셸처럼 정확히 읽게 고치는 길도 있지만, 그러면 **자르는 쪽에 셸 파서가 또 하나**
//   생긴다 - 이 파일이 여덟 회차 동안 구멍을 낸 자리가 바로 그 이음매다. 그래서 반대로 간다:
//   **못 읽었으면 온 명령으로 되돌린다**(이 파일의 fail-closed 규율). 세 갈래:
//   (1) **따옴표 짝이 안 맞다** = 따옴표 안에서 잘랐다. 실측: `rm -rf "x&" /` · `rm -rf 'a;b' /` ·
//       `rm -rf "a|b" ~` · `rm -rf "a>b" /` · 따옴표 안 개행.
//   (2) **리다이렉션(`<`·`>`)에서 잘랐다.** 셸에서 리다이렉션은 인자 **가운데**에도 올 수 있어
//       그 뒤가 여전히 인자다. 실측: `rm -rf ./tmp >/dev/null *` · `rm -rf ./tmp >/dev/null /`.
//   (3) **자른 글자 바로 앞이 역슬래시였다** = 그 글자는 연산자가 아니라 글자다.
//       실측: `rm -rf ./tmp \; /`.
//   대가는 정직하게 적는다: 이름에 `&`·`;`·`|`·`<`·`>` 가 든 파일을 지우거나 꼬리를 붙이면서
//   명령 다른 자리에 위험 글자가 있으면 과차단이 난다(옛 판도 똑같이 막던 자리라 새 손해는 아니다).
...
function rmHitsDangerTarget(cmd) {
  ...
  for (const m of s.matchAll(RM_RECURSIVE_G)) {
    ...
    const cut = rest.match(RM_ARGS_END);
    const args = cut ? rest.slice(0, cut.index) : rest;
    const unreadable = rmArgsUnreadable(args)
      || (!!cut && /[<>]/.test(cut[0]))                              // (2) 리다이렉션에서 잘랐다
      || (!!cut && cut.index > 0 && rest.charAt(cut.index - 1) === "\\");  // (3) 앞이 역슬래시였다
    // 🛑 **줄 이음(`\` + 개행)은 자리가 아니다.** 토큰으로 세기 **전에** 뗀다. 안 떼면 외톨이 `\` 가
    //   아래에서 "모르는 자리"로 잡혀 `rm -rf \`(개행)`./build && cd /` 가 도로 과차단된다.
    let sawName = false, sawUnknown = false;
    for (const tok of args.replace(/\\\n/g, " ").trim().split(/\s+/)) {
      const kind = rmTokenKind(tok);
      if (kind === "top") return true;               // 이 rm 이 꼭대기를 지운다
      else if (kind === "name") sawName = true;
      else if (kind !== "skip") sawUnknown = true;   // 플래그는 모름이 아니다(위 `rmTokenKind` 주석)
    }
    // 🛑 되돌림을 끄는 조건은 **"이름이 보인다"가 아니라 "이 구간을 다 읽었다"**이다.
    //   이름 한 자리가 모르는 자리까지 덮으면 안 된다: `-exec rm -rf ./tmp//x {} +` 의 `{}` 는
    //   뿌리 아래 모든 파일로 펼쳐지는데, 앞의 `./tmp//x` 하나 때문에 되돌림이 꺼졌다(실측).
    // 못 읽었거나(위 세 갈래) · 모르는 자리가 있거나 · 이름이 하나도 없으면 옛 판대로 온 명령을 본다.
    //   정적으로 푼 것이 아니라 안전측에 머무는 것이라, 앞 세그먼트 글자로 과차단이 날 수 있다.
    if ((!sawName || sawUnknown || unreadable) && RM_DANGER_TARGET.test(s)) return true;
  }
  return false;
}
...
//   - **앞 세그먼트가 뿌리로 옮겨 간 뒤 상대 경로를 지우는 것**(`cd / && rm -rf ./build`)은
//     이제 **통과한다**(의도한 것이다 · 2026-08-14 검수 판정). 옛 판의 차단은 정책이 아니라
//     옆자리 슬래시에 걸린 부수 효과였다: `rm -rf /build` 를 직접 쳐도 옛 판은 안 막았다.
//     즉 "뿌리 밑 이름 붙은 폴더"는 애초에 위험 집합에 없다(그건 다른 판이 정할 축이다).
//   - **플래그를 띄어 쓴 재귀삭제**(`rm -r -f /` · `rm -r -f ~`)는 `RM_RECURSIVE` 무늬에 애초에
//     안 걸려 이 함수까지 오지도 않는다(옛 판도 같다 · 실행으로 확인). 위 그 상수 주석 참고.
...
//   ⚠ **증상**: 여기 못 들어간 앞머리 플래그를 쓰면 그 줄이 **다시 막힌다**(각본 파일이 뒤에 있어도
//   첫 낱말에서 접히기 때문). 정상 작업이 갑자기 막히면 이 목록을 먼저 보라. 푸는 방법은 목록을
//   늘리는 것이 아니라 **그 플래그가 표준입력을 코드로 안 읽는다는 실측 근거를 먼저 만드는 것**이다.
...
function pipesIntoInterpreter(s) {
  ...
  PIPE_TARGET.lastIndex = 0;   // 전역 정규식은 상태를 들고 다닌다: 누가 test 를 부르면 시작점이 밀린다
