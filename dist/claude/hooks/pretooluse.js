...
process.stdin.on("end", () => {
  ...
  try {
    ...
    //
    // 🛑 **서브에이전트에는 안 띄운다**(2026-08-20). 사정이 둘이고, 각각 혼자서도 끄는 이유가 된다.
    //   (1) 판정이 읽는 `input.transcript_path` 는 서브에이전트에서 **부모(메인) 기록**이다
    //       (위 '감독 상한이 읽을 자리' 주석의 2026-08-12 실측 · 그 상한이 자기 기록 경로를
    //       따로 조립하는 이유가 이것이다). 그래서 이 절은 "이 일꾼이 계획서를 썼나"가 아니라
    //       **"메인이 계획서를 고쳤나"**를 묻는다: 계획서를 한 번도 안 쓴 일꾼이 메인의 계획서
    //       편집 때문에 편집할 때마다 이 안내를 받는다(2026-08-20 재현 · 일꾼 둘이 실제로 보고).
    //   (2) 문구가 시키는 일을 서브에이전트는 **할 수가 없다**: 게이트 스폰은 같은 파일의
    //       `subagentGateSpawn` 이 탈출구 없이 막는다. 시키는 대로 하면 막히고 안 하면 안내가
    //       계속 뜬다 - 따를 수 없는 지시라, 기록을 옳게 읽도록 고쳐도 서브에이전트에는 여전히
    //       안 띄우는 것이 맞다. 게이트는 본 세션이 띄운다는 규율이 이미 그 자리를 덮는다.
    ...
    //   routing 리마인더(§4.5) 하나뿐인데, 그 절은 **위임(Agent 스폰) 자리에서만** 켜진다.
    //   구현 일꾼 둘은 중첩 스폰을 계약으로 안 하니 거기서 끝이지만, **감독(`supervisor`)은 그 계약의
    //   예외라 실제로 띄운다**(`src/skills/routing/SKILL.md` 5항 · 감독 쓰기 차단 `supervisorBlock` 은
    //   Write·Edit·MultiEdit·NotebookEdit·Bash 만 막고 `Agent` 는 안 막는다). 즉 정상 경로에서
    //   **도달은 한다** - 조용한 이유는 계약이 아니라 판정이다: `routingReminderNeeded` 의
    //   "이미 위임 시작(1회 보장)" 침묵이, 부모(메인) 기록에 남은 **메인의 감독 스폰 한 줄**로 이미
    //   켜져 있다(감독이 `DELEGATE_AGENTS` 에 들어 있다). 그래서 미뤄도 구멍이 안 생긴다.
    //   🛑 그러니 감시할 것은 두 일꾼의 계약이 아니라 **`routingReminderNeeded` 의 1회 보장**이다:
    //   그 침묵을 손대면(예: 자기 기록만 보게 고치면) 이 절·§4.7 과 같은 갈래를 그때 단다.
    //   🛑 그 침묵은 **세 겹**이다(같은 함수 안): `Skill`+`routing` 로드 검사 · 위 "1회 보장" ·
    //   그 "1회 보장" 이 감독을 세려면 있어야 하는 `DELEGATE_AGENTS` 의 `supervisor`. 규칙을 지킨
    //   세션은 첫 겹에 먼저 걸리고, 방아쇠는 셋 중 **둘 이상이 풀릴 때** 당겨진다.
    //   왜 값나가는가: 감독의 `tools:` 에는 `Skill` 이 없어서, 그 안내가 도달하면 "Skill 도구로
    //   로드하라"는 **따를 수 없는 지시**가 된다 - 이번 판이 §4·§4.7 에서 없앤 바로 그 병이다.
