...
- plan-validator **NO-GO/CONDITIONAL**, pr-reviewer **BLOCK/REQUEST CHANGES** → treat as high/blocker, **stop**. CONDITIONAL never means "fix while implementing" — report conditions, get approval, register as finish-check scoring items. **Re-validating a plan is not an answer to a verdict** — only for blockers you actually fixed; CONDITIONAL, or blockers 0 and it keeps returning, goes to the user (a loop nobody decides burns days). Code differs: BLOCK/REQUEST CHANGES fixes **are** re-reviewed before PR (hook enforces).
