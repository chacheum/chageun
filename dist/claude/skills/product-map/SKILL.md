...
> **화면 없는 제품은 IA를 만들지 않는다.** CLI 도구·플러그인·라이브러리·API처럼 사용자 화면(UI)이 없는 제품은 그릴 화면이 없으므로 `docs/ia-structure.md`를 생성하지 않고 **기능 명세(산출물 1)만** 관리한다. 이 경우 feature-spec의 "관련 화면" 칸은 `화면 없음`으로 적는다.
