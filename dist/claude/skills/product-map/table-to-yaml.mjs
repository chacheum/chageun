...
// 🛑 아래 집합 마지막 항목(긴 줄표 U+2014)은 **판정 값**이라 일괄 치환 대상이 아니다(v0.67.0 T7).
//    이 주석에 그 글자를 다시 적지 않는 이유: 코어 규칙(operating-rules.md:41)이 "소스에 남은 긴
//    줄표는 전부 판정 값이다"라고 적고 있어, 그 글자를 설명글에 쓰면 그 문장이 거짓이 된다.
//    옛 지도 표의 우선순위 칸이 긴 줄표 한 글자인 줄이 있고, 그 줄을 닻으로 인식해 복원하려고
//    집합에 넣어 둔 값이다.
//    하이픈으로 바꾸면 `"-"` 가 두 번 들어가 실질 항목이 하나 줄고, `:152` 의 닻 찾기가 실패해
//    `:153` 의 "닻을 못 찾았다"로 빠진다. 지키는 검사 = test/table-to-yaml.test.mjs 의
//    "우선순위 칸이 긴 줄표 한 글자인 밀린 줄도 닻으로 인정한다".
