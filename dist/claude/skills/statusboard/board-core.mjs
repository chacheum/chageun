...
// 자리표는 `<##N##>` 모양을 쓴다: esc() 가 이미 돌아 원문의 맨 `<`·`>` 는 전부 `&lt;`·`&gt;`
// 로 바뀐 뒤라, 이 시점부터는 바 `<`·`>` 문자가 원문에서 나올 수 없다 (D-4: 예전엔
// ` N `처럼 숫자만으로 된 자리표를 써서, 원문에 그냥 있던 맨 숫자가 되돌리는 단계에서
// 코드 조각으로 뒤바뀌었다).
// ⚠ 이 자리표 모양을 바꾸면 `test/statusboard-core.test.mjs` 의 "자리표를 흉내 낸 원문"
// 검사 입력(`<##0##>`)도 함께 바꿔라 - 그 입력은 이 모양을 손으로 베낀 사본이라, 모양이
// 바뀌면 그 검사는 평범한 글자를 재는 셈이 되어 빨간불 없이 계속 통과한다.
