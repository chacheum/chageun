...
NO-GO는 재검증을 부르지만 **회차 상한이 없으면 끝나지 않는다.** 실제로 한 계획이 8차까지 갔고
(매 회차 서로 다른 새 blocker), 그 사이 계획서는 개정 로그가 본문의 4분의 1을 차지할 만큼 커져
**다음 검증기가 더 큰 문서를 읽고 새 지적을 내는 되먹임**이 돌았다.

**회차 파악:** 호출 프롬프트에 회차가 있으면 그것을 쓴다. 없으면 **계획서의 개정 로그 절 개수로 추정**하고,
그것도 없으면 1차로 본다. (추정이 1차 경로다 — 재검증은 보통 routing 스킬이 안 떠 있는 자리에서 일어난다.)

**3차 이후에도 새 blocker/high가 나오면**, 판정과 함께 다음을 **§1에 medium 이상 finding으로 올린다**:
- 계획이 한 번에 검증하기엔 크거나 설계가 아직 안 정해졌다는 신호다. **더 작은 단위로 쪼개** 각각
  검증하거나, 남은 판단을 사람에게 올리라고 권고한다.
- 회차마다 **새로운** blocker가 나오는지, **같은** 것이 반복되는지 구분해 적는다. 전자는 계획이 큰 것이고,
  후자는 수정이 지적을 안 짚은 것이다 — 처방이 다르다.
이 권고는 **차단이 아니라 에스컬레이션**이다. 그것만을 이유로 판정을 NO-GO로 올리지 않는다.
