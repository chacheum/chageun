...
- **분량은 내용에 맞춘다** — 담을 것은 다 담되 **채우기용 절·중복 요약·상투 문구로 부풀리지 않는다**.
  위 규모 보고가 켜졌을 때 커진 큰 이유가 내용이 아니라 부풀림이면, 같은 보고 안에서 그 점을 짚어
  "쪼개기"가 아니라 "덜어내기"를 제안한다. **부풀림만으로는 별도 finding을 만들지 않는다**(400줄 아래면
  이 축으로 아무 말도 하지 않는다): 게이트가 분량을 지적할수록 계획자가 그 지적에 답하는 절을 덧붙여
  계획서가 더 커지는 되먹임이 생긴다(v0.46.0에서 회차 지적 누적으로 같은 되먹임을 이미 한 번 막았다).
