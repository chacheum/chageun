...
실제 구동 검증의 절차는 이 세션에 인라인된 `run-verify` 스킬 본문(운영 규칙 뒤 첨부)을 따릅니다 — 절차를 여기서 중복 기술하지 않습니다. Codex 도구 매핑만: Claude 전용 `run`·`verify` 스킬 대신 shell로 dev 서버를 기동(백그라운드) → **Playwright**(있으면)로 화면·동작 확인, 없으면 `curl`로 응답·상태코드라도 확인하고 "시각 확인 제한됨" 명시. **테스트 환경·운영 쓰기 hard-block은 플랫폼 무관으로 그대로**(격리 환경 없으면 보류·운영 시도 금지). Playwright·dev 기동 불가 환경이면 "동작 검증 안 됨 — 미검증 출시 위험"으로 **보고만** 합니다.
