...
const REASONS = {
  "plan-size":
    "차단: 계획서가 3,000줄을 넘습니다. 이 크기부터는 검수 신뢰도가 떨어져 승인을 받고 진행합니다 — " +
    ...
    "**판단은 이 메시지 맨 끝 `잰 파일`의 줄 수를 보고 하세요.** 3,700줄 아래면 그 구간엔 잘 끝난 " +
    "계획도 있어(3,017 · 3,080 · 3,456 · 3,523 · 3,651줄) 승인 후 진행이 맞을 수 있습니다. " +
    "그보다 크면 알려진 성공 사례가 없으니 일을 쪼개는 쪽이 안전합니다.\n" +
    ...
    "**그 질문 본문에 잰 파일 이름과 실제 줄 수, 3,700과 비교한 결과를 한 줄로 적으세요** " +
    "(예: `my-plan.md는 3,200줄입니다 — 이 구간엔 잘 끝난 계획도 있습니다`). 이 차단문은 사용자가 " +
    "아니라 당신이 읽습니다 — 안 적으면 사용자 화면엔 승인 키의 천 단위 버킷(`3k`)만 남습니다. " +
    "**아래 형식 요건과 달리 이건 기계가 확인하지 않습니다**(줄 수 표기를 기계로 대조하면 " +
    "`3,200`↔`3200` 같은 차이로 승인이 영영 인정 안 되는 오차단이 납니다).\n" +
...
// ⚠ **정정(2026-08-09, Fable 독립 심판)**: 처음 근거로 적었던 "잘 끝난 계획 최대 2,560줄"은 **틀렸다.**
//   12개 표본이 그때 진행 중이던 성공작을 놓쳤다. 전 프로젝트를 다시 세니 **3,017 · 3,080 · 3,456 ·
//   3,523 · 3,651줄 계획이 모두 코드를 출하했다**(해당 기간 커밋으로 확인). 즉 3,000은 알려진 성공
//   크기보다 아래이고, 이 가드는 **멀쩡한 큰 계획도 막는다.** 문턱을 올릴지는 사용자 결정으로 남겼다
//   (올리면 실측 사고 4,020줄도 안 걸린다 — 맞바꿈이다).
...
//   "3,700 아래면 승인 후 진행이 맞을 수 있습니다"가 뜻을 잃는다 — 그 아래는 이제 안 막히므로).
