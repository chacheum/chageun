...
const REASONS = {
  "plan-size":
    ...
    "계획도 있고(3,017 · 3,080 · 3,456 · 3,523 · 3,651줄) 그보다 크면 알려진 성공 사례가 없습니다. " +
    "다만 실측 사고는 **1건**뿐이라(4,020줄 계획이 10회 넘게 재검증되며 나흘간 코드 0줄) " +
...
//   말 것 — 그 주장은 **데이터가 반증한다**(3,017줄·3,651줄 계획이 코드를 출하했다). 이 가드가
//   말하는 것은 하나뿐이다: **"이 크기부터는 검증 결과를 믿기 어려우니, 계속 갈지 사람이 정한다."**
//   틀려도 잃는 것이 승인 클릭 하나라 예측이 맞을 필요가 없고, 그래서 표본 1건으로도 설 수 있다.
//   프레임을 예측기로 되돌리면 성공 사례 2건이 즉시 반례가 되고, "표본이 한 자릿수라 문턱 근거가
...
//   - 3,000을 넘긴 계획서 3개 중 2개(3,017 · 3,651)는 **코드를 출하했다.** 즉 문턱은 알려진 성공
//     크기보다 아래에 있다. 그래도 되는 이유는 위 "동의 관문" 절이다.
//   - 실제 차단 이력: **1회**(오차단 0). 표본이 1건이라 오차단률은 사실상 미측정이다.
//   - 가드가 처음부터 있었다면 걸렸을 멀쩡한 작업은 한 세션(승인 질문 최대 2번)뿐이다.
//   - 경보 피로는 반대편에서 관측됐다 — 계획 12개 중 10개에 울리던 400줄 소프트 경보가 무시됐다.
//     하드 차단은 164개 중 3개꼴이라 그 빈도가 아니다.
