 ...
 *      ②③ 은 이번 세션에 안 풀리는 갈래(무시 판정 `blocked` 는 캐시가 세션 내내 굳는다)라
 *      **미루지 않고 그 자리에서 내린다**: 켜 두면 이 문이 세션 내내 열린 채가 되어, 한 글자도
 *      못 쓰는 프로젝트에서 도구 호출마다 파싱 비용만 나간다. 내려도 잃는 것은 없다 - 표는
 *      조각이 아니라 장부 전체로 다시 그리므로 다음 소식 때 그 변화도 함께 나간다.
