...
- plan-validator **NO-GO/CONDITIONAL**, pr-reviewer **BLOCK/REQUEST CHANGES** → treat as high/blocker, **stop**. CONDITIONAL never means "fix while implementing" — report conditions to the user, get approval, register as finish-check scoring items. **Re-running the gate is not an answer to a verdict.** Rewriting the plan and validating again is only for blockers you actually fixed; for CONDITIONAL, or when blockers are 0 and the plan keeps coming back, take it to the user instead — a loop where nobody decides burns days.
