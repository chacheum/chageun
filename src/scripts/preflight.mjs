...
const SECRET_ENV = /(SECRET|TOKEN|PASSWORD|PRIVATE_KEY|_KEY)$/i;
const PAID_VALUE = /\bsk_live_|\bsk-[A-Za-z0-9]|AKIA[0-9A-Z]{16}/; // stripe live·openai·aws 등
...
export function evaluate(config, isAlive, env) {
  ...
  for (const [k, v] of Object.entries(env || {})) {
    if (SECRET_ENV.test(k) || PAID_VALUE.test(String(v))) reasons.push(`위험 환경변수(시크릿/키) 감지 — 무인 부적합: ${k}`);
  }
...
export function main() {
  ...
  const isAlive = (name) => {
    try { return execFileSync("docker", ["inspect", "-f", "{{.State.Running}}", name], { encoding: "utf8" }).trim() === "true"; }
