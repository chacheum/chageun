...
제품의 기능과 화면 구조를 한 곳에 모아 두어, 기획 대화·계획·구현의 모든 단계에서 같은 기준으로 참조하고 갱신한다. 지도가 최신 상태를 유지하면 기능 누락·이름 불일치·화면 중복 같은 혼선을 방지할 수 있다.
...
기획 대화·계획·구현 **각 단계 시작 시** 다음 두 파일을 읽어 기존 제품 맥락을 파악하고 새 작업을 일관되게 이어간다.
...
- 각 건마다 사용자에게 3택: **무시 / `chageun:debugging`으로 파보기 / 백로그**(백로그로 고른 건만 feature-spec `비고`나 메모로 남김).
...
- **(버그 의심이면) 파보기 → `chageun:debugging`** - 단순 누락이 아니라 코드가 잘못 동작하는 것으로 의심되면 파본다(🔎 "지도화 중 발견된 의심"과 같은 선택지: 경로가 달라도 출구는 동일).
