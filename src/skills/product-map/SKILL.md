...
🛑 **v0.60.0 으로 이미 옮겼으면 한 번 되짚는다.** 그 판은 밀린 줄을 되살릴 때 `|` 양옆에 공백을 넣어 `?open=a|b` 를 `?open=a | b` 로 바꿔 놓고도 대조 세 줄이 전부 ✅ 였다(v0.61.0 에서 고침). 그때 `⚠ 알아 둘 것` 에 이름이 오른 줄만 `git show <옮기기 직전 커밋>:docs/feature-spec.md` 로 원본 표를 꺼내 `|` 주변 공백을 대조하면 된다. 영향받는 줄은 그 줄들뿐이다.
