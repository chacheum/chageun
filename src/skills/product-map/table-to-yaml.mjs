...
// **원본과 `|` 주변 공백이 달라졌으면** 그 버그가 되살아난 신호다.
//
  ...
  for (const row of rows) {
    ...
    if (cells.length === COLS.length) {
      ...
      if (!rowRestoreOk(row, parts))
        ...
        fatal.push(`${f.id}: 이 줄은 끝 칸이 빠진 것 같다 — 줄 끝에 빈 칸 하나(\`| |\`)를 더 넣어 ` +
                   `이름표 ${COLS.length}칸을 다 채워라. 설명·비고에 \`|\` 를 일부러 썼다면 칸이 ` +
                   `${COLS.length}개보다 많은 게 정상이니 **그 \`|\` 는 지우지 마라**`);
      else
