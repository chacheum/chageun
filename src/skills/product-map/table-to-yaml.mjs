...
// v0.61.0 에서 고쳤다 — 이제 안쪽 공백을 안 건드린다. 그러니 옮긴 결과에서 이 모양이 보이면
// **정상이 아니라 그 버그가 되살아났다는 신호**다(옛 주석은 이걸 "고칠 수 없는 예외"라고 적어 뒀었다).
//
  ...
  for (const row of rows) {
    ...
    if (cells.length === COLS.length) {
      ...
      if (!rowRestoreOk(row, parts))
        fatal.push(`${f.id}: 이 줄은 칸 수가 안 맞는다(비고 같은 끝 칸이 빠졌을 가능성이 크다) — ` +
                   `그 줄의 \`|\` 개수를 세어 칸이 8개가 되게 손으로 맞춰라`);
      else
