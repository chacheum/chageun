...
const PR_MARKERS = [
  ...
  // v0.68.0 작업방 위임: 코드가 작업방에 있는데 본 폴더를 보면 결과가 **빈 화면이 아니라
...
const CI_MARKERS = [ // code-implementer(감사 지적: 마커 0개 → 표류 못잡음)
  ...
  // v0.68.0 작업방 위임: 일꾼이 작업방에서 돌면 본 폴더의 `git diff` 는 빈 결과다.
...
const DI_MARKERS = [
  ...
  // v0.68.0 작업방 위임: 형제(code-implementer)와 한 벌이다. 한쪽만 고치면 이 저장소 단골 사고다.
...
const SV_MARKERS = [
  ...
  // v0.68.0 작업방 위임(10-1·10-2): 감독에게는 `git` 이 없다. 스스로 확인할 수 없으므로
