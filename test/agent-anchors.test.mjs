...
const PV_MARKERS = [
  ...
  "실구동 검증·끝 점검은 별도 축으로 세지 않는다", // 2회차 high 1: 좁히개가 planning 쪽에만 있고 게이트에는
                                  // 없었다. 코어가 이 둘을 **의무로** 요구하므로(operating-rules 의 real-run ·
                                  // finish check), 이 예외가 없으면 돌아가는 것을 만드는 계획서가 거의 다
                                  // 걸려 늘 울리는 경보가 된다(400줄 가드가 12개 중 10개에 울려 무시된 전례)
