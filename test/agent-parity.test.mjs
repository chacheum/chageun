...
const PR_MARKERS = [
  ...
  "애매하면 붙이지 않는다",      // v0.40.0: [정리] 태그의 안전판(오태그 시 사용자 눈에서 사라짐)이 한 플랫폼에서만 지워지는 표류 방어
  "앞머리를 붙이지",             // v0.41.0: git 읽기 하나만(cd·echo·2>/dev/null 금지) 안내가 한 플랫폼에서만 지워지는 표류 방어
  ...
  "3차 도달이 blocker를 면제하지", // v0.48.0 안전-핵심. ⚠Codex 쪽은 plan-validator 문단이 이미 이 리터럴을
                                 // 갖고 있어 이 앵커만으론 pr-reviewer 절 삭제를 못 잡는다 —
...
const PV_MARKERS = [
  ...
  "그것만을 이유로 판정을 올리지", // v0.46.0 과잉차단 방지: 이 절이 codex에서만 지워져 같은 계획이 플랫폼마다
                                  // 다른 판정을 받던 표류를 기계가 못 잡았다(pr-reviewer medium)
  ...
  "`재검증 회차: N` 옆에 적힌",   // v0.47.0: 지난 회차 지적의 출처. 한쪽에서만 지워지면 그 플랫폼의
                                  // 오탐 가드가 자기 데이터에 못 닿아 같은 지적 반복도 "새 지적"이 된다
  "얕게 본 부분을 명시",          // 4번: 계획 규모 가드의 정직 고지절(조용한 축소 금지)
  "애매하면 붙이지 않는다",      // v0.40.0: [정리] 태그의 안전판이 codex에서만 지워지는 표류 방어
];
