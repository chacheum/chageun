...
// 2026-08-09 +183 (20404 → 20587): **CONDITIONAL 출구 수리**(Fable 독립 심판 최우선 권고).
// 실측 사고에서 게이트가 CONDITIONAL 을 **7번** 냈는데 착수는 0번이었다 — 나흘 중 사흘이 blocker 0
// 상태였다. 원인은 판정을 받고 **게이트를 다시 돌린 것**인데, "재검증은 판정에 대한 답이 아니다"라는
// 문장이 어디에도 없었다. plan-validator `:98`("구현 중 해결 가능")과 코어 `:53`("멈춰라")이 서로
// 반대라 메인 세션이 다시 검증받는 쪽을 골랐다. 게이트 문서도 같이 고쳤다.
// **회수 −110B 를 처음으로 실제로 했다**(3차 리뷰 low C): `:21` 의 훅 동작 설명 괄호를 줄였다.
// 같은 이유가 바로 위 장부 항목에 이미 적혀 있어 코어에서 두 번 값을 치르고 있었다. 순증은 +183 이다.
// ⚠ 재핀 횟수 정정: 앞 항목의 "6번째"는 틀렸다. v0.54.0 브랜치에서만 3회(19992→20083→20206→20404)
// 했으므로 이번이 **9번째**다. 회수 후보(LIGHT/FULL 통합)는 그 두 줄이 커져 502B → 약 690B 다.
const CEILING_BYTES = 20587;
