...
//    그물은 "빗나가" 한 문투만 잡으면 안 된다: "세 번 실패하면" · "3회 시도 후" · "셋째 시도까지는" ·
//    "세 번째로 빗나가면" 이 전부 3회 문턱인데 옛 정규식은 넷 다 통과시켰다. 그래서 서술어를
//    `빗나가|실패|시도` 로 넓히고, 수량 표현에 `세 번째` 를 넣고, 사이에 조사·꾸밈말이 끼어도 잡히게
//    간격을 허용한다("세 번째" + "로 " + "빗나가").
//    🛑 오탐은 **그물을 좁혀서** 피하지 않는다. 옛 판이 "번 바로 뒤가 서술어"로 좁혀 정상 문장
//       ('두 번 빗나가면 멈춘다' 문장과 같은 줄의 "세 번째 시도 대신")을 피했는데, 그 좁힘 때문에
//       진짜 3회 문턱이 "세 번째로
//       빗나가면"으로 쓰이면 그대로 통과했다. 허용 문구는 아래 ALLOWED 로 빼서 **본문 사본에서 먼저
//       지운 뒤** 잰다 — 그러면 허용 문구는 애초에 후보에서 빠지므로 그물을 좁힐 이유가 없다.
//       검사에 맞춰 본문을 고치는 쪽으로도 풀지 않는다.
...
const ALLOWED = [
  // '두 번 빗나가면 멈춘다' 문장과 같은 줄. 2회에서 멈춘 뒤 "세 번째 시도"를 **하지 않고** 사용자에게 넘긴다는 문장이라
  // 3회 문턱의 정반대다(문턱 자체는 같은 줄의 "두 번 빗나가면 멈춘다" 앵커가 지킨다).
  "세 번째 시도 대신",
];
