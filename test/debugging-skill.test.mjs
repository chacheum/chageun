...
// `chageun:debugging` 은 걷어낸 `systematic-debugging` 자리를 대신 받는다. 코어 '작업 유형별 진행'
// 절의 'Bug / failing test' 줄은 포인터 문투(`load ... via the Skill tool`)를 **안 쓰기로** 했으므로,
// 이 스킬 본문을 지키는 그물은 이 파일뿐이다.
...
//    그물은 "빗나가" 한 문투만 잡으면 안 된다: "세 번 실패하면" · "3회 시도 후" ·
//    "셋째 시도까지는" · "세 번째로 빗나가면" 이 전부 3회 문턱인데 옛 정규식은 넷 다
//    통과시켰다. 그래서 서술어를 `빗나가|실패|시도` 로 넓히고, 수량 표현에 `세 번째` 를
//    넣고, 사이에 조사·꾸밈말이 끼어도 잡히게 간격을 허용한다("세 번째" + "로 " + "빗나가").
//    🛑 오탐은 **그물을 좁혀서** 피하지 않는다. 옛 판이 "번 바로 뒤가 서술어"로 좁혀
//       정상 문장('두 번 빗나가면 멈춘다' 문장과 같은 줄의 "세 번째 시도 대신")을
//       피했는데, 그 좁힘 때문에 진짜 3회 문턱이 "세 번째로 빗나가면"으로 쓰이면 그대로
//       통과했다. 허용 문구는 아래 ALLOWED 로 빼서 **본문 사본에서 먼저 지운 뒤** 잰다 —
//       그러면 허용 문구는 애초에 후보에서 빠지므로 그물을 좁힐 이유가 없다. 검사에
//       맞춰 본문을 고치는 쪽으로도 풀지 않는다.
// 🛑 ALLOWED 를 늘릴 때는 **왜 그 문장이 3회 문턱이 아닌지**를 그 줄에 함께 적는다. 근거 없이 한 줄
//    더하면 그 문구만큼 그물이 조용히 뚫린다(오탐 하나를 없애려다 진짜 3회 문턱까지 함께 통과시킨다).
//    지운 자리 앞뒤가 붙어 새 문구가 생길 수 있으니, 예외를 더하면 이 테스트를 한 번 부러뜨려 본다.
const ALLOWED = [
  // '두 번 빗나가면 멈춘다' 문장과 같은 줄. 2회에서 멈춘 뒤 "세 번째 시도"를
  // **하지 않고** 사용자에게 넘긴다는 문장이라 3회 문턱의 정반대다(문턱 자체는
  // ANCHORS 의 "두 번 빗나가면 멈춘다" 가 지킨다).
  "세 번째 시도 대신",
];
const THREE_STRIKE = /(세 번째|세 번|3번째|3번|3회|삼 번|셋째)[^\n]{0,6}?(빗나가|실패|시도)/g;
test("debugging 의 멈춤 문턱이 2회다(3회 문투 회귀 차단)", () => {
  let probe = skill;
  for (const allowed of ALLOWED) probe = probe.split(allowed).join("");
  const three = probe.match(THREE_STRIKE) || [];
...
// 🛑 배선 축: 스킬을 만들어 놓고 **가리키는 자리를 안 고치면** 아무도 이 스킬에 못 온다.
//    옛 이름(`systematic-debugging`)이 남아 있으면 그 스킬이 없는 사용자에게 "설치하라"는
//    막다른 길이 배포된다. 그래서 **양방향**으로 잰다(옛 이름 부재 + 새 이름 존재).
const WIRED = [
  ["rules/operating-rules.md", join("rules", "operating-rules.md")],
  ["skills/run-verify/SKILL.md", join("skills", "run-verify", "SKILL.md")],
  ["skills/monitoring/SKILL.md", join("skills", "monitoring", "SKILL.md")],
  ["skills/product-map/SKILL.md", join("skills", "product-map", "SKILL.md")],
];

test("디버깅을 가리키는 4자리가 새 이름을 쓰고 옛 이름이 안 남았다", () => {
  for (const [label, rel] of WIRED) {
    const txt = readFileSync(join(ROOT, "src", rel), "utf8");
    assert.ok(!txt.includes("systematic-debugging"),
      `${label} 에 옛 이름 systematic-debugging 이 남았다 — 없는 스킬로 보내는 막다른 길이다`);
    assert.ok(txt.includes("chageun:debugging"),
      `${label} 이 chageun:debugging 을 안 가리킨다 — 이 스킬에 닿을 길이 없다`);
  }
});
