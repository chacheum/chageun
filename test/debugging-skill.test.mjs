...
//    이 전부 3회 문턱인데 옛 정규식은 셋 다 통과시켰다. 그래서 서술어를 `빗나가|실패|시도` 로 넓힌다.
//    🛑 `\s*` 를 더 느슨하게(예: `[^\n]{0,4}`) 풀지 말 것: 본문 39행의 정상 문장 "세 번째 시도 대신"
//       이 걸려 오탐이 된다("세 번" + "째 " + "시도"). 지금은 "번" 바로 뒤가 서술어여야 잡히므로
//       그 문장은 안 걸린다. 검사에 맞춰 본문을 고치는 쪽으로 풀지 않는다.
test("debugging 의 멈춤 문턱이 2회다(3회 문투 회귀 차단)", () => {
  const three = skill.match(/(세 번|3번|3회|삼 번|셋째)\s*(빗나가|실패|시도)/g) || [];
