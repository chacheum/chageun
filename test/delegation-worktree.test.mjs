...
const ROUTING_WORKTREE_PARTS = [
  ...
  // 🛑 4회차: **값이 이미 증명된 규칙인데 그물 밖이었다.** 이 대조가 첫 실전에서 실제로 사고를
  //   막았다 - 일꾼이 뒤처진 방을 받고 여기 걸려 파일을 한 개도 안 건드리고 멈췄다. 그런데도 이
  //   항목을 통째로 지우고 돌리면 검사가 전부 초록이었다(실측): 앵커가 없고, 하위 항목이라
  //   아래 개수 칸(`numberedItems` 가 번호 항목만 센다)에도 안 잡혀 WORKTREE_SECTION_ITEMS 가
  //   그대로다.
  //   두 줄을 잡는 이유는 **문구가 물러지는 것**까지 재려는 것이다 - 앞엣것만 재면 판정이
  //   "0이 아니면 옛 코드"에서 "다르면 참고한다"로 물러져도 초록이다.
  "방을 만든 직후 기준점을 대조한다",
  "0이 아니면 그 방은 옛 코드",
