...
test("extractGates: pr-reviewer APPROVE 추출", () => {
...
test("extractGates: plan-validator는 NO-GO를 GO보다 우선 매칭", () => {
  const objs = [gateCall("t2", "plan-validator"), gateResult("t2", "권고: NO-GO. 이유는 GO 조건 미충족")];
  assert.equal(extractGates(objs)[0].verdict, "NO-GO");
});

test("extractGates: 게이트가 아닌 Task는 무시", () => {
  const objs = [gateCall("t3", "general-purpose"), gateResult("t3", "APPROVE")];
  assert.deepEqual(extractGates(objs), []);
});

test("extractGates: 판정 문자열 없으면 unknown", () => {
  const objs = [gateCall("t4", "pr-reviewer"), gateResult("t4", "리뷰 진행 중...")];
