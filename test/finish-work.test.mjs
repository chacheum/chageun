...
// 당시 회고 탐지기는 이미 이 형태를 갭으로 셌는데 훅만 못 잡아 실측 10건 중 7건이 어휘로 샜다.
// (회고는 그 뒤 제거됐다. 넓힌 어휘 판정은 훅에 남으므로 이 검사도 그대로 남는다.)
...
// ── v0.42.1: 실구동 주장 오탐 봉합(실사용 첫 세션에서 훅이 스스로를 오차단했다) ──
test("실구동: 조건·가정·요청 어미면 주장이 아니라 서술 → 침묵(v0.42.1 오차단 봉합)", () => {
  const desc = [
    "끝 점검·실구동 검증·요약을 절차 없이 대충 끝내면 이제 실제로 되돌려집니다.",   // 실제 오차단 문장
    "실구동까지 통과하면 머지와 push를 승인해 주세요.",
    "실구동 검증까지 끝낼까요?",              // 축약 의문 (pr-reviewer medium)
    "실구동 검증이 통과돼야 머지합니다.",       // 축약 조건
    "구동 검증을 완료해야 합니다.",            // 축약 의무
    "실구동 주장을 미로드로 끝내면 되돌린다.",  // 실측 코퍼스 형태(창 안 — 전방탐색이 실제로 작동)
  ];
  for (const t of desc) {
    assert.equal(shouldBlockSkillGap([U("x"), A(t)]), null, "서술문: " + t.slice(0, 24));
  }
});
test("실구동: 진짜 완료 주장은 계속 잡는다(미탐 회귀 방지)", () => {
  const claims = [
    "실구동 검증 완료.",
    "실구동 검증 통과했습니다.",
    "실구동 검증은 방금 배포본으로 7/7 완료했습니다.",
    "실제로 띄워서 확인했습니다.",
    "구동 검증 — 7/7 통과",
    "실구동 검증 끝났습니다.",
    "실구동 검증 끝났으니까 push합니다.",      // `[가-힣]*까` 일반화 금지 근거 — 이건 진짜 주장
  ];
  for (const t of claims) {
    assert.equal(shouldBlockSkillGap([U("x"), A(t)]), "run-verify", "주장문: " + t.slice(0, 24));
  }
});

// 한계 정직 고지: 조임은 **조건·가정 어미**만 본다. "…끝내는 경우를 막습니다" 같은 관형형 서술은
// 여전히 걸린다. 관형형 "-는"까지 넓히면 "완료했는데 문제가 있었습니다" 같은 진짜 주장을 놓치므로
// 의도적으로 안 넓혔다(미탐보다 이 오탐이 싸다).
test("실구동: 관형형 서술은 여전히 오탐 — 알려진 한계", () => {
  assert.equal(shouldBlockSkillGap([U("x"), A("실구동 주장을 스킬 미로드로 끝내는 경우를 막습니다.")]), "run-verify",
    "이 모양은 아직 못 거른다(넓히면 진짜 주장을 놓쳐서 의도적 보류)");
  assert.equal(shouldBlockSkillGap([U("x"), A("실구동 검증 완료했는데 한 군데가 이상합니다.")]), "run-verify",
    "관형형까지 넓혔다면 이 진짜 주장을 놓쳤을 것");
});
