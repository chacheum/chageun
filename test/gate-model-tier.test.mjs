...
// 모델을 메인으로 상시 돌리면 게이트가 메인보다 약해진다. 코어 `# Model · execution routing` 절의
// 원칙 문장(심판은 검토 대상보다 약하면 안 된다)은 **지금도 그대로 있고**, v0.58.0이 그 뒤에 비용 우선
// 예외(`Omit \`model\` even if you outrank the gate`)를 붙여 그 예외가 이기게 했다. 그래서 이 테스트는
// **마이그레이션 체크포인트**다:
