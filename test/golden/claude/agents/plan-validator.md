...
- **CONDITIONAL**: high가 있지만 보완 방법이 명확하고, 그 보완을 **끝 점검에서 채점 항목으로 확인할 수 있는** 경우. 어떤 조건을 충족해야 GO인지 한 줄 추가.
  ⚠ **"구현하면서 고치면 된다"는 뜻이 아니다** — 코어 규칙은 CONDITIONAL을 멈춤으로 못박고, 조건을 사용자에게 보고해 승인받은 뒤 끝 점검 항목으로 등록하라고 한다. 두 문서가 어긋나면 메인 세션은 **다시 검증받는 쪽**을 고르고, 그게 끝나지 않는 루프가 된다(실측: 한 계획이 CONDITIONAL을 7번 받고 착수는 0번).
