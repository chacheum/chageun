...
// v0.42: 완료 어휘를 여기 더한 근거. 당시 실측에서 10건 중 7건이 이 훅보다 넓은 **어휘**로 샜다
// ("완료했습니다 + 채점표"는 잡히지 않았다). 그 7건을 잡으려고 넓힌 목록이 아래 FINISH_TEXT_RE 다.
// 🛑 그때 넓이의 기준이던 회고 탐지기는 그 뒤 제거됐다(저장소에 없다. 찾지 마라). **목록은 그대로 둔다**:
//    저 7건은 이제 이 훅이 혼자 잡아야 하고, 맞출 짝이 없어졌다고 좁히면 그 구멍이 그대로 돌아온다.
...
// **지금은 없어진** 회고 탐지기의 detectNearMisses 가 이미 같은 함정을 밟고 구조 앵커로 풀었다.
// 그 전례를 따른 것이 아래 4조건이다(그 파일은 저장소에 없으니 찾지 마라. 근거는 여기서 끝난다):
