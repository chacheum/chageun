...
- **Delegated implementation always runs in its own worktree** (`Agent(isolation: "worktree")`; **unattended sessions are exempt: they already run in an isolated clone and must not create one**). Read its diff as `git -C <path> diff <base>...HEAD` **plus `status --porcelain`**: workers commit, so a bare diff is empty and a gate passes it silently, while uncommitted work still shows in `status`. Never claim real-run verification before that worktree has its env and deps. **Nothing counts as done until main merges that branch back into the main line** (`git -C <main folder> merge <branch>`): until then the user's own files are unchanged, however green the report looks. **Cannot create one → not a git repo: propose `git init` first (never inside a monorepo subfolder: a nested `.git` silently detaches that subtree); tool error: report that and delegate without a worktree.** Either way record that it ran without one, and never fall back to inline silently. (Details: `chageun:routing`.)
- **Main never edits product code inline** (product code = app · library · test sources; **where the product ships as documents, those shipped files too**): it plans, delegates, verifies. It still writes plans, docs, memory, `status.md`, and repo housekeeping files (ignore lists, settings) itself.
