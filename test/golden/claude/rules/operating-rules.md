...
- plan-validator **NO-GO/CONDITIONAL**, pr-reviewer **BLOCK/REQUEST CHANGES** → treat as high/blocker, **stop**. CONDITIONAL never means "fix while implementing" — report conditions to the user, get approval, register as finish-check scoring items. **Re-validating a plan is not an answer to a verdict** — only for blockers you actually fixed; CONDITIONAL, or blockers 0 and it keeps returning, goes to the user (a loop nobody decides burns days). Code differs: BLOCK/REQUEST CHANGES fixes **are** re-reviewed before PR (hook enforces).
...
Gates, planning, specs, architecture, complex judgment, final review: **the top-tier reasoning model — never a model below the main session's** (a judge weaker than the worker it reviews defeats the point). Omit `model` even if you outrank it (cost).
