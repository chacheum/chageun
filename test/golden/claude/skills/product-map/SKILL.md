...
**따옴표 규칙:** 값 안에 `: `(콜론+공백)이나 ` #` 가 있거나, 값이 `-` `?` `:` `[` `{` `#` `*` `!` `|` `>` `'` `"` `%` `@` 중 하나로 시작하면 **큰따옴표로 감싼다**(`설명: "가격: 10% 할인"`). 안 그러면 YAML 이 다른 뜻으로 읽는다. 여러 줄이 필요하면 `설명: |-` 뒤에 들여쓰기해 적는다.
...
손으로 옮기지 않는다 — 옮기다 한 줄 빠뜨려도 아무도 모른다. 딸린 스크립트를 쓴다.

```
node "${CLAUDE_PLUGIN_ROOT}/skills/product-map/table-to-yaml.mjs" docs/feature-spec.md
```

`--write` 없이 돌리면 **결과만 보고하고 파일은 안 건드린다.** 칸 복원 대조·되돌림 대조가 둘 다 ✅ 이면 그때 `--write` 를 붙여 실제로 옮긴다. 한 군데라도 어긋나면 스크립트가 **아무것도 쓰지 않는다**(내용 유실 방지). 표 밖 텍스트(검증 체크리스트 등)는 손대지 않는다. 이미 칸이 밀려 있던 줄은 `우선순위`+`상태` 를 닻으로 복원하고 `⚠ 알아 둘 것` 으로 보고하니, 그 줄은 사람이 눈으로 확인한다.
