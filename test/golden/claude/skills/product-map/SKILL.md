...
> **표가 아니라 YAML 인 이유.** 마크다운 표는 `설명`·`비고` 안에 `|` 가 한 글자만 들어가도 그 뒤 칸이 통째로 한 칸씩 밀린다. 그런데 밀려도 표는 멀쩡히 그려져서 아무도 모른다 — 실측 2026-08-10, 실제 프로젝트 6곳 455개 기능 중 10개가 밀린 채였고 `화면` 칸에 `비고` 내용이 들어가 있었다. YAML 은 칸마다 이름이 붙어 있어 이 사고가 구조적으로 안 생긴다.
