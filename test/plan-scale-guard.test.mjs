...
test("차단문 세 벌의 문턱 숫자가 상수와 같다", () => {
  ...
  //   ⚠ 이 단언은 위 `:79`(< 3651)에 포섭돼 **단독으로는 안 울린다** — 값은 문턱을 올리며 `:79`를
  //   사실에 맞게 고쳐 초록으로 만든 **뒤에도** 한 번 더 걸리는 두 번째 알림이라는 점이다.
  //   "중복이네" 하고 지우지 말 것(5회차 low).
...
//   그걸 지키는 게 주석의 🛑 경고뿐이면 빌드를 못 깬다. 이 파일 `:130-137` 이 이미 같은 방식으로
//   plan-validator 산문을 앵커한다(저장소 관례). 표현을 다듬을 때 함께 고쳐야 하는 것은 노린 효과다.
test("차단문이 예측 주장이 아니라 동의 관문으로 말한다", () => {
  ...
  assert.match(human, /검증 결과를 믿기 어려/,
    "멈추는 이유(검증을 믿기 어렵다)가 빠지면 남는 건 '크니까 막는다'뿐이라 근거가 사고 1건짜리 " +
      "일화로 되돌아간다");
