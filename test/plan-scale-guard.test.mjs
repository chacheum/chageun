...
//   나도 그걸 확인했다며 좁혔는데, **둘 다 틀렸다.** (나)와 예문은 `어렵…` 활용형이라 둘째 음절이
//   `렵`(U+B835)이고, 정규식이 찾는 `려`(U+B824)를 포함하지 않는다. 그래서 옛 정규식도 첫 문장
//   하나에만 걸렸다. 내 "확인"은 `reasonFor()` 결과가 아니라 **소스 파일 전체**에 정규식을 대본
//   것이었다(서브에이전트 문구·주석이 걸려 true 가 나왔다).
