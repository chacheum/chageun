...
//   그걸 지키는 게 주석의 🛑 경고뿐이면 빌드를 못 깬다. 아래 "plan-validator 400줄은 보고 문턱"
//   테스트가 이미 같은 방식으로 산문을 앵커한다(저장소 관례). 표현을 다듬을 때 함께 고쳐야 하는
//   것은 노린 효과다.
// ⚠ **단언마다 그 문장에만 있는 형태로 좁힌다.** 6회차 medium: `/검증 결과를 믿기 어려/` 로 쟀더니
//   차단문 안에 세 번(첫 문장 · (나) 요구 · 예문) 나와서, 잠그려던 첫 문장을 지워도 초록이었다.
//   변이 주입 3건에서 이 단언만 빠져 있었던 것이 그 신호였다 — 변이를 못 만들겠으면 그 단언은
//   아직 아무것도 안 잠근 것이다.
test("차단문이 예측 주장이 아니라 동의 관문으로 말한다", () => {
  ...
  assert.match(human, /통으로 못 읽어 \*\*검증 결과를 믿기 어려워지므로/,
