...
const ANCHORS = [
  ...
  "쪼갤지 먼저 본다",                    // 2번: 큰 요청의 세부부터 다듬어 통째로 버리는 것 방어.
                                        // 제목이 "너무 크면"이었을 때는 **크기**가 적용 조건처럼 읽혀,
                                        // 작지만 축이 섞인 요청에서 이 절을 통째로 건너뛰었다(2회차 low)
  ...
  // 🛑 2번의 "확인 방법" 축이 **왜** 필요한지는 본문에서 뺐다(2회차 low): 본문은 기획 대화마다 로드되는
  //    자리라 행동을 바꾸는 조건만 남긴다. 근거는 여기 둔다 - 종이 위에서는 두 축의 규칙과 시험 픽스처가
  //    서로 부딪히는 것을 아무도 못 본다. 앞 축을 먼저 넣고 나면 그 시험칸이 **돌아가는 사실**이 되어,
  //    뒤 축은 종이가 아니라 기계와 대조된다.
  ...
  "축으로 안 세는 것 둘",                // 위 축의 좁히개. 코어는 실구동 검증과 끝 점검을 **의무로**
                                        // 요구하므로(operating-rules 의 real-run · finish check),
                                        // 이 예외가 없으면 돌아가는 것을 만드는 계획서는 거의 다 걸린다.
                                        // 같은 실패가 이 저장소에 이미 있다: 400줄 가드가 계획서 12개 중
                                        // 10개에 걸려 늘 울리는 경보가 됐고 그래서 무시됐다
                                        // (pretooluse-core.js 의 기준선 근거 주석)
  "코어가 의무로 요구하는 실구동 검증·끝 점검은 별도 축으로 세지 않는다", // 위 앵커는 좁히개의 **머리말만**
                                        // 붙든다. 실제로 오탐을 막는 것은 (ㄴ) 이 조각이라, 이게 없으면
                                        // 머리말만 남기고 예외를 지워도 초록이다(2회차 low)
  "태스크마다 **무엇으로 확인하나**",     // 3회차 low: 역선택 막기의 planning 쪽 짝. 게이트는 확인 방법이
                                        // 안 적힌 계획서에서 이 축을 켜지 않는다(안 적은 쪽이 이득을 보지
                                        // 않게). 그 면제가 성립하려면 **적으라는 요구**가 여기 살아 있어야
                                        // 한다. 짝(plan-validator 의 "확인 방법이 아예 안 적혀 있으면 이
                                        // 축을 켜지 말고")은 test/agent-anchors.test.mjs 가 지킨다 -
                                        // 한쪽만 지워지는 것이 이 저장소의 단골 사고다
];
