...
// **여유는 그 값보다 작아야 한다.** 조각 4의 여유 1,663자 + 역대 최대 덩어리 1,674자 = 8,011 로
// 8,000을 넘어 빨간불이 뜨고, 물리 천장 10,000에는 한참 못 닿는다(7,105 + 1,674 = 8,779).
// 즉 "빨간불은 떴지만 규칙은 멀쩡히 닿는" 자리에서 잡히고, 검사를 지나쳐 조용히 잘리는 경로가 없다.
// ⚠ 방향을 헷갈리지 마라. 여유가 1,674자보다 **크면** 역대 최대 덩어리가 그대로 들어가 버려
//   빨간불이 안 뜬다 — 이 줄의 앞선 판(머리말에 조각 명단을 넣기 전, 여유 1,695자)이 실제로
//   그 상태였고 주석은 반대로 적혀 있었다(pr-reviewer 3항 반영 중 재측정하다 발견).
//   여유가 이 값을 다시 넘으면 쪼개기를 다시 보거나 조각 경계를 조여라.
