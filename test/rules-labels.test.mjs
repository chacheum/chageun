...
const SECTION_LABELS = [
  ...
  // (이 검사는 "절 제목 + 소실 방지 앵커"라 영어 앵커가 자연스럽다). 코어의 이 줄이 빠지면
  // '작업 유형별 진행' 절의 'Bug / failing test' 줄에서 `chageun:debugging` 으로 가는
  // 유일한 입구가 사라진다(아래 "코어 '작업 유형별 진행'이 `chageun:debugging` 을 가리킨다"
  // 검사와 같은 축이며, 그 검사가 실제로 잡는 것은 `chageun:debugging` 은 남기고 라벨 문구만
  // 갈아 끼우는 개정이다).
  "Bug / failing test",
];
