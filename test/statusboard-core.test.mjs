...
// ⚠ `<##0##>` 는 board-core.mjs 의 `inline()` 함수가 쓰는 실제 자리표 모양을 손으로 베낀 사본이다.
...
// 빨간불로 만드는 것은 맨 숫자 쪽이고, 그 축은 바로 위 "inline: 코드 조각 뒤에 맨 숫자가
// 있으면 숫자가 그대로 남는다 (자리표 충돌)" 검사가 이미 잰다.
...
// 보안 바닥인데 위쪽 "md: 스크립트가 이스케이프되고 링크 문법이 안 산다" 검사는 백틱 밖만 본다.
