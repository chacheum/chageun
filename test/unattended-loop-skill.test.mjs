...
const MARKERS = [
  ...
  // 큐 마커 셋(queue.md · 위에서부터 · 커밋 이력)은 2026-08-19 에 뺐다.
  // 런처가 큐를 방으로 안 옮긴다(chageun-unattended:68 이 task.md·criteria.md 둘만 복사).
  // 문서가 약속하고 코드가 배달을 안 해서 문서를 "한 번에 한 가지"로 좁혔다.
  // 다시 넣으려면 런처부터 고친다.
